...
#from Scrapers.Weather.Weather import Weather
import threading
...
def run():
    ...
    for i in range(num_items):
        ...
        """
        {
            "_id": ObjectId("570fda451ab65051a014f533"),
            "home_team_place": null,
            "away_team_place": null,
            "score_home": null,
            "result": null,
            "score_away": null,
            "score_away_ht": null,
            "score_home_chances": null,
            "home_team": "Aalesund",
            "away_team_players": {},
            "audience": null,
            "est_player_score_home": null,
            "vg_scoring_away": null,
            "stadium": {
                "lat": 62.469744,
                "lon": 6.187394,
            ...
            "rain": 0,
            "temperature": -9999.999999,
            "web_id": 811908,
            "est_player_score_away": null,
            "home_team_players": {},
            "date": ISODate("2016-03-11T19:00:00.000Z"),
            "away_team": "Stabæk",
            "vg_scoring_home": null,
            "score_home_ht": null,
            "wind": -9999.999999
        }
        """
        ...
        """
        db_stadium, created = Model.Stadium.create_or_get(name=match_data["stadium"])
        db_team_away, created = Model.Team.create_or_get(name=match_data["away_team"])
        db_team_home, created = Model.Team.create_or_get(name=match_data["home_team"])
        db_match, created = Model.Match.create_or_get(id=parameter_list["matchId"], home_team=db_team_home, away_team=db_team_away, stadium=db_stadium)

        db_match.id = parameter_list["matchId"]
        db_match.away_team = db_team_away
        db_match.home_team = db_team_home
        db_match.stadium = db_stadium
        db_match.score_home = match_data["home_score"]
        db_match.score_away = match_data["away_score"]
        db_match.score_home_ht = match_data["home_halftime_score"]
        db_match.score_away_ht = match_data["away_halftime_score"]
        db_match.game_date = match_data["game_date"]
        db_match.result = match_data["result"]
        db_match.division = parameter_list["tournamentId"]
        db_match.last_updated = datetime.datetime.now()
        """


        # Run Weather fetching
        """w = Weather()
        w.retrieve_weather(db_match)
        t = threading.Thread(target=w.retrieve_weather, args = (db_match,))
        t.daemon = True
        t.start()"""

        # Fetch match information and players
        #
